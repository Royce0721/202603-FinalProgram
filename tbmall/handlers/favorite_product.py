...
def favorite_product_list():
    ...
    order_by = (
        FavoriteProduct.id.asc()
        if order_direction == 'asc'
        else FavoriteProduct.id.desc()
    )
    ...
    total = query.count()
    favorite_products = query.order_by(order_by).limit(limit).offset(offset).all()

    return json_response(
        favorite_products=FavoriteProductSchema().dump(favorite_products, many=True),
        total=total
    )
